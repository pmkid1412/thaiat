...
/**
 * Vietnamese labels for marks
 */
const MARK_LABELS_VI: Record<Mark, string> = {
  bold: "In đậm",
  italic: "In nghiêng",
  underline: "Gạch chân",
  strike: "Gạch ngang",
...
/**
 * Gets the formatted mark name in Vietnamese
 */
export function getFormattedMarkName(type: Mark): string {
  return MARK_LABELS_VI[type] || type.charAt(0).toUpperCase() + type.slice(1)
}
